...
| `gif_frames_kb` | census only, never evicted: whole decoded GIF frame lists. Since #201 only GIFs that actually RENDER transparency build one -- the rest play off the shared mp4 tile cache and show up under `video_readers_kb` instead -- and each is capped by `DECKARD_GIF_KEY_BUDGET_MB` (default 32), over-budget streaming into a bounded alpha-dropped cache variant. (With `performance.cache-videos` off nothing has a cache to route to, so this column carries every GIF on screen.) So this column now tracks a bounded quantity (≤ budget × alpha GIF keys); what it still sizes is whether an AGGREGATE cap across alpha GIFs is ever warranted. Before #201 it was the largest un-capped image holder in the app: a 32-key page of 200-frame GIFs is ~0.9 GiB, roughly 10x the entire budget the ceiling governs. |
