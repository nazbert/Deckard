        ...
        for position in ("top", "center", "bottom"):
            for alignment in ("left", "center", "right"):
                for outline_width in (0, 2, 4):
                    ...
                    # The first render populates the cache and the rest replay
                    # it, and every one must be byte-identical.
...
def check_label_edits_invalidate() -> None:
    ...
    try:
        ...
        # The in-place editor path bypasses the setters and relies on
        # invalidate_scroll_caches().
        ...
        # The explicit reset sites drop both caches.
        _render(lm, size, bg)
...
def check_labelless_key_identity() -> None:
    ...
    try:
        ...
        # this key must carry no label for the identity contract.
        lm.clear_labels()
...
def check_mutation_round_gaps() -> None:
    ...
    try:
        ...
        # 1. Both setters must drop the recorded blits, not only call
        # invalidate_scroll_caches() and clear_labels().
        ...
        # 2. Multiline labels. text() masks each line separately, so the replay
        # has to reproduce a whole sequence of blits in order.
        lm.clear_labels()
        ...
        # 3. The touch image of the dial runs the same label stage, identity
        # early-out included, through a caller that never copies.
