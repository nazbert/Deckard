...
of the dead accessor chain fails this test.
"""
...
def _fake_app(visible_child, sidebar_update):
    ...
    return SimpleNamespace(
        main_win=SimpleNamespace(
            ...
            sidebar=SimpleNamespace(update=sidebar_update),
        )
    )
...
def main() -> None:
    ...
    try:
        ...
        # 3. No UI at all (headless / early boot) -> silent no-op.
        gl.app = None
        controller.update_ui_on_page_change()

        print("PASS: sidebar refreshes for the visible deck only, no dead chains")
    finally:
        gl.app = saved_app
