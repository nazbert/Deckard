...
- **Opaque keys' first paint on a background‑video page** (issue #11 / audit B‑11, `ba45d412`) — `update_all_inputs`'s bg‑video branch now `update()`s keys whose composed background colour is fully opaque instead of skipping them, and re‑pushes the in‑app preview for the rest (`2366b00c`). This is what retires the "self‑heals within one tick" claim; see the corrected disposition below.
...
- Minor: with a background video set, `update_all_inputs` still leaves the *device* paint of **non‑opaque** keys to the media loop (`DeckController.py:1399‑1441`), pushing those keys to the in‑app preview only. **The earlier "skipped keys self‑heal within one 33 ms tick" wording is retired — it was false for opaque keys.** The per‑frame loop deliberately never repaints a key whose composed background colour is fully opaque (its tile hides the video, so nothing changes frame to frame), so after a switch onto a bg‑video page those keys kept the *previous* page's imagery until a keypress (issue #11; fixed in `ba45d412` — they are no longer skipped at all, so the opaque case is now covered by an explicit paint rather than by healing). What holds today: the keys still skipped are exactly the ones `ControllerKey.on_media_player_tick` repaints (`:4888‑4920` — repaints iff the composed alpha is < 255), so their heal is bounded by the **video write budget**, not by the loop tick — `_video_write_hz`, default 30 Hz ⇒ ≤ ~33 ms, and the bound moves with `DECKARD_VIDEO_WRITE_HZ` (`:387`, `:655‑674`). It does not hold at all while the opt‑in quiescence gate (issue #144) is engaged: the whole animation section is skipped, and those keys are painted instead by the gate's settle window on the next page generation or pending repaint (`:533‑538`, `:568‑641` — `GATE_SETTLE_TICKS` = 3 quiet ticks, hard‑bounded by `GATE_WINDOW_MAX_S` = 0.5 s).
