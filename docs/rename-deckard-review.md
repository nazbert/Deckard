...
- **App-ID literal sprawl** (altitude): `io.github.nazbert.Deckard` appears ~25× across 12 Python files in 4 spellings (dotted, slash-path, ayatana-underscore, suffixed) plus `DEFAULT_DATA_PATH` is a dead re-derivation of `gl.DATA_PATH`. `rebrand_migration.py` proves a stdlib-only, pre-`globals`-importable `appinfo.py` constants module is viable for every consumer — worth extracting before the next id change repeats this grep.
