...
| `KeyGIF` frames (post-fix, #201) | opaque GIFs play off the shared mp4 tile cache (O(1) frame, `video_readers` census); only alpha-carrying GIFs keep an RGBA frame list, fitted to 2× tile (P2.3) and capped per GIF by `DECKARD_GIF_KEY_BUDGET_MB` (default 32, over-budget degrades to the opaque route) | ≤32MB per alpha GIF key; ~0 for opaque ones *(was 41–200MB per GIF key, all frames at source resolution)* |
